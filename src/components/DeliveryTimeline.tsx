...
export function DeliveryTimeline({ batch, confirmations }: DeliveryTimelineProps) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'received_confirmed': return 'bg-[#00C5E9]';
      case 'delivery_confirmed': return 'bg-[#3F76FF]';
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-base">
          <Clock className="h-5 w-5 text-[#3F76FF]" />
          Timeline da Entrega
        </CardTitle>
      </CardHeader>
        ...
        <div className="border-t pt-3 space-y-4">
            ...
            <div className="flex gap-3">
              <div className="flex flex-col items-center">
                <div className="rounded-full bg-blue-100 p-2">
                  <Truck className="h-4 w-4 text-[#3F76FF]" />
                </div>
                <div className="w-px bg-gray-300 flex-1 min-h-8" />
              </div>
              <div className="flex-1 pb-4">
          ...
          {deliveryConfirmation && (
            <div className="flex gap-3">
              <div className="flex flex-col items-center">
                <div className="rounded-full bg-blue-100 p-2">
                  <Camera className="h-4 w-4 text-[#3F76FF]" />
                </div>
              ...
              <div className="flex-1 pb-4">
                ...
                <p className="text-xs text-gray-600">{getUserById(deliveryConfirmation.confirmedByUserId)?.name}</p>
                ...
                {deliveryConfirmation.notes && (
                  <p className="text-xs text-gray-600 mt-1 italic">"{deliveryConfirmation.notes}"</p>
                )}
              </div>
            </div>
          )}
            ...
            <div className="flex gap-3">
              <div className="flex flex-col items-center">
                <div className="rounded-full bg-cyan-100 p-2">
                  <CheckCircle className="h-4 w-4 text-[#00C5E9]" />
                </div>
              </div>
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-600">{getUserById(receiptConfirmation.confirmedByUserId)?.name}</p>
                ...
                {receiptConfirmation.notes && (
                  <p className="text-xs text-gray-600 mt-1 italic">"{receiptConfirmation.notes}"</p>
                )}
              </div>
            </div>
          )}
