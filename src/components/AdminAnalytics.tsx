...
export function AdminAnalytics() {
  ...
  const masterLog = useMemo(() => {
    ...
    movements.forEach(m => {
      ...
      logs.push({
        id: `mov-${m.id}`,
        timestamp: new Date(m.movementDate),
        type: 'movement',
        action: m.type === 'in' ? 'Entrada de Estoque' : 'Saída de Estoque',
        user: user?.name || 'Desconhecido',
        userRole: user?.role || '',
        item: item?.name || 'Item desconhecido',
        quantity: Math.abs(m.quantity),
        unit: unit?.name || '',
        status: m.type === 'in' ? 'Entrada' : 'Saída',
        details: `${m.type === 'in' ? '+' : '-'}${Math.abs(m.quantity)} ${item?.name} - ${m.reason || 'Sem motivo especificado'}`,
      });
    });
    ...
    requests.forEach(r => {
      ...
      if (r.status === 'rejected' && r.approvedAt && approver) {
        logs.push({
          ...
          unit: unit?.name || '',
          status: translateStatus(r.status),
          details: `${approver.name} rejeitou solicitação de ${r.quantity} ${item?.name}${r.rejectionReason ? ` - Motivo: ${r.rejectionReason}` : ''}`,
        });
      }
    ...
    deliveryBatches.forEach(batch => {
      const driver = getUserById(batch.driverId);
      
      logs.push({
        id: `batch-${batch.id}`,
        timestamp: new Date(batch.createdAt),
        type: 'delivery',
        action: batch.status === 'delivered' ? 'Lote Entregue' : 'Lote Criado',
        ...
        details: `${driver?.name} ${batch.status === 'delivered' ? 'entregou' : 'criou'} lote com ${batch.requestIds.length} solicitações`,
      });
    });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold" style={{ color: COLORS.primary }}>{stats.movements}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold" style={{ color: COLORS.secondary }}>{stats.requests}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold" style={{ color: COLORS.warning }}>{stats.transfers}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold" style={{ color: COLORS.success }}>{stats.approvals}</div>
          </CardContent>
        </Card>
      </div>
