...
export function AddStockDialog({ 
  ...
  const { currentUser, currentUnit, addMovement, unitStocks, getItemById, generateUniqueProductsForStock } = useApp();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (stockId) {
      ...
      // Se for produto único, gerar IDs automaticamente
      const item = getItemById(stock.itemId);
      if (item?.isUniqueProduct) {
        generateUniqueProductsForStock(stock.itemId, stock.unitId, qty);
      }
    } 
    // Caso 2: Ainda não existe stock (itemId foi passado)
      ...
      if (item?.isUniqueProduct) {
        generateUniqueProductsForStock(itemId, currentUnit.id, qty);
      }
    } else {
